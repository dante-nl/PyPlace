...
# 𝗖𝘂𝗿𝗿𝗲𝗻𝘁 𝘃𝗲𝗿𝘀𝗶𝗼𝗻
# Default: 0.8 (changes every version)
...
def UpdateCheck():
	...
	response = requests.get("https://pyplace.dantenl.com/version.json")
...
{data["release_notes"]}
		...
		while NotAnswered == True:
			...
			if Answer == "y":
				...
				r = requests.get(
					"https://pyplace.dantenl.com/PyPlace-Latest.py", allow_redirects=True)
				if not r.ok:
...
URLToPythonFile)
			...
			StoreRequest = requests.get("https://pyplace.dantenl.com/store.json", allow_redirects=False)
			...
			ExperimentRequest = requests.get(
				"https://pyplace.dantenl.com/experiments.json", allow_redirects=True)
	...
	while NotAnswered3 == True:
		...
		if str(Answer3) == "1":
			...
			if Answer3.lower().startswith("-install"):
				...
				response = requests.get(f"https://pyplace.dantenl.com/orders/{order_name}/manifest.json")
